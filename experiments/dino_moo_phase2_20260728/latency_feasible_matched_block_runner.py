...
"""Run one complete feasible-cohort matched-latency block on eight GPUs."""
...
def validate_plan(plan: dict[str, Any]) -> None:
    ...
    if not isinstance(candidates, list) or len(candidates) != 4:
        raise ValueError("block plan must contain exactly four candidates")
...
def load_latency_stats_module(path: Path, expected_sha256: str) -> Any:
    ...
    module_name = "_dino_latency_feasible_matched_latency_stats"
