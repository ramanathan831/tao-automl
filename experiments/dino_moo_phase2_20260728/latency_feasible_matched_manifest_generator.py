...
"""Generate the immutable 98%-accuracy-feasible latency manifest.

The complete selector analysis is independently recomputed by the
manifest-pinned ``tao_automl`` selector from every successful objective record
in the sealed candidate table.  Generation fails unless that complete replay
exactly matches the combined-selection audit and each table audit.  The
validation cohort is then derived solely by the frozen selector predicate
``valid and latency_accuracy_feasible``.  Exact checkpoints and full model
records are joined from the same independently hashed table.  No new latency
measurement, winner identity, or candidate desirability participates in
scheduling.
...
DEFAULT_OUTPUT = HERE / "latency_feasible_matched_manifest.v1.json"
...
EXPECTED_LATENCY_RETENTION_TYPE = "relative"
EXPECTED_LATENCY_RETENTION_VALUE = 0.98
EXPECTED_ACCURACY_REFERENCE_CANDIDATE_ID = "seed_271828_rec_18"
EXPECTED_ACCURACY_REFERENCE_VALUE = 0.6554138278683255
EXPECTED_ACCURACY_THRESHOLD = 0.6423055513109589
EXPECTED_FEASIBLE_CANDIDATE_IDS = (
    "seed_161803_rec_14",
    "seed_271828_rec_16",
    "seed_271828_rec_18",
    "seed_314159_rec_12",
)
EXPECTED_FEASIBLE_CANDIDATE_SET_SHA256 = (
...
def latency_feasible_audits(
    analysis: dict[str, Any],
) -> tuple[list[dict[str, Any]], dict[str, dict[str, Any]]]:
    """Return the independently replayed frozen 98%-feasible cohort."""

    audits = analysis.get("candidates")
    if not isinstance(audits, list) or not audits:
        raise ContractError("selector analysis candidates must be non-empty")
    by_id: dict[str, dict[str, Any]] = {}
    for audit in audits:
        if not isinstance(audit, dict):
            raise ContractError("selector analysis candidate audit is invalid")
        candidate_id = audit.get("candidate_id")
        if not isinstance(candidate_id, str) or not candidate_id:
            raise ContractError("selector audit lacks candidate_id")
        if candidate_id in by_id:
            raise ContractError(f"duplicate selector audit: {candidate_id}")
        by_id[candidate_id] = audit

    algorithm = analysis.get("algorithm")
    if not isinstance(algorithm, dict):
        raise ContractError("selector analysis algorithm is missing")
    configuration = algorithm.get("configuration")
    if not isinstance(configuration, dict):
        raise ContractError("selector configuration is missing")
    retention = configuration.get("latency_accuracy_retention")
    require_equal(
        retention,
        {
            "type": EXPECTED_LATENCY_RETENTION_TYPE,
            "value": EXPECTED_LATENCY_RETENTION_VALUE,
            "reference": "accuracy_winner",
            "reference_candidate_id": None,
            "reference_value": None,
        },
        "frozen latency accuracy-retention configuration",
    )
    for key, expected in (
        (
            "accuracy_reference_candidate_id",
            EXPECTED_ACCURACY_REFERENCE_CANDIDATE_ID,
        ),
        ("accuracy_reference_value", EXPECTED_ACCURACY_REFERENCE_VALUE),
        ("accuracy_threshold", EXPECTED_ACCURACY_THRESHOLD),
        (
        ...
        (
            ...
            EXPECTED_ACCURACY_REFERENCE_VALUE,
        ),
        ("latency_accuracy_threshold", EXPECTED_ACCURACY_THRESHOLD),
    ):
        require_equal(algorithm.get(key), expected, f"selector algorithm {key}")
    ...
    cohort = sorted(
        (
            ...
            and audit.get("latency_accuracy_feasible") is True
        ),
        key=lambda item: item["candidate_id"],
    )
    candidate_ids = [item["candidate_id"] for item in cohort]
    require_equal(
        candidate_ids,
        list(EXPECTED_FEASIBLE_CANDIDATE_IDS),
        "frozen 98%-feasible candidate IDs",
    )
    require_equal(
        sha256_value(candidate_ids),
        EXPECTED_FEASIBLE_CANDIDATE_SET_SHA256,
        "frozen 98%-feasible candidate-set SHA256",
    )
    for audit in cohort:
        if audit.get("pareto_rank") not in (0, 1):
            raise ContractError(
                f"{audit['candidate_id']} has unexpected global Pareto rank"
            )
        dominated_by = audit.get("dominated_by")
        if not isinstance(dominated_by, list):
            raise ContractError(
                f"{audit['candidate_id']} dominated_by must be a list"
            )
    return cohort, by_id
...
def derive_candidate_records(
    ...
    cohort, _ = latency_feasible_audits(replay_analysis)
    combined_cohort, _ = latency_feasible_audits(combined)
    require_equal(
        cohort,
        combined_cohort,
        "derived replay/combined latency-feasible audits",
    )
    require_equal(
        combined.get("selections", {}).get("latency", {}).get(
            "latency_tied_candidate_ids"
        ),
        list(EXPECTED_FEASIBLE_CANDIDATE_IDS),
        "frozen selection-time equivalent-fastest cohort",
    )
    rows = table.get("rows")
    if not isinstance(rows, list) or not rows:
    ...
    for audit in cohort:
        ...
        if row is None:
            raise ContractError(
                "latency-feasible candidate lacks retained record: "
                f"{candidate_id}"
            )
        require_equal(
            row.get("status"),
        ...
        record = {
            ...
            "global_pareto_rank": audit.get("pareto_rank"),
            "global_dominated_by": copy.deepcopy(audit.get("dominated_by")),
    ...
    require_equal(
        ...
        list(EXPECTED_FEASIBLE_CANDIDATE_IDS),
        "canonical frozen latency-feasible candidate order",
    )
    require_equal(
        sha256_value([item["candidate_id"] for item in records]),
        EXPECTED_FEASIBLE_CANDIDATE_SET_SHA256,
        "derived latency-feasible candidate-set SHA256",
    )
    return records
...
def build_schedule(candidate_ids: list[str]) -> dict[str, Any]:
    ...
    for allocation_index, design_index in enumerate(selected_indices):
        ...
        allocation = {
            "allocation_id": (
                f"latency_feasible_allocation_{allocation_index:02d}"
            ),
...
def build_manifest(
    *,
    ...
    require_equal(
        candidate_ids,
        list(EXPECTED_FEASIBLE_CANDIDATE_IDS),
        "manifest frozen 98%-feasible candidate IDs",
    )
    require_equal(
        sha256_value(candidate_ids),
        EXPECTED_FEASIBLE_CANDIDATE_SET_SHA256,
        "manifest frozen 98%-feasible candidate-set SHA256",
    )
    replay_cohort, _ = latency_feasible_audits(replay_analysis)
    require_equal(
        [item["candidate_id"] for item in replay_cohort],
        candidate_ids,
        "manifest/replay latency-feasible candidate IDs",
    )
    ...
    allocation_contract["matching"] = (
        "Every allocation benchmarks every frozen 98%-accuracy-feasible "
        "candidate exactly once on the same node; candidate-pair differences "
        "are matched within allocation."
    )
    allocation_contract["incomplete_allocation_policy"] = (
        "Exclude the entire allocation and rerun the complete cohort under a "
        "new TAO job ID; never combine a partial block."
    )
    manifest = {
        "schema_version": 1,
        ...
        "candidate_derivation": {
            ...
            "predicate": (
                "recomputed valid is true and recomputed "
                "latency_accuracy_feasible is true"
            ),
            "latency_accuracy_retention": {
                ...
                "reference_value": EXPECTED_ACCURACY_REFERENCE_VALUE,
                "threshold": EXPECTED_ACCURACY_THRESHOLD,
            },
            "canonical_order": "ascending UTF-8 candidate_id",
            ...
            "global_rank_zero_candidate_ids": copy.deepcopy(
                replay_proof["global_rank_zero_candidate_ids"]
            ),
            "frozen_feasible_cohort_exact_match": True,
            "selection_time_tied_cohort_exact_match": True,
        },
        "selection_snapshot": {
            "algorithm": copy.deepcopy(combined["algorithm"]),
            "selections": copy.deepcopy(combined["selections"]),
            "selection_authority": copy.deepcopy(
                combined["selection_authority"]
            ),
            "candidate_audit_count": len(combined["candidates"]),
            "candidate_audits_sha256": sha256_value(
                combined["candidates"]
            ),
        ...
        "runtime": {
            ...
            "local_runtime_path": str(
                (HERE / "runtime" / "latency_feasible_matched").resolve()
            ),
        ...
        "paired_analysis": {
            **copy.deepcopy(post["paired_analysis"]),
            "comparison_set": (
                "all unordered pairs of frozen 98%-accuracy-feasible "
                "candidates"
            ),
            "practical_tolerance_ms": EXPECTED_PRACTICAL_TOLERANCE_MS,
        ...
        "selection_isolation": {
            **copy.deepcopy(post["selection_isolation"]),
        ...
        "incomplete_allocation_policy": allocation_contract[
