...
"""Read-only aggregation of latency-feasible matched measurements.
...
before result loading, but matched measurements are never passed to a selector
and cannot modify selection-time objectives or winner identities.
"""
...
DEFAULT_OUTPUT = DEFAULT_RUNTIME / "latency_feasible_matched_analysis.json"
EXPECTED_ALLOCATIONS = 6
EXPECTED_RANKS = 8
EXPECTED_CANDIDATES = 4
FROZEN_LATENCY_WINNER_ID = "seed_271828_rec_18"
...
def require_selection_isolation(
    value: dict[str, Any],
    label: str,
) -> None:
    """Require every validation-only isolation claim to be explicit and false."""
...
def load_ledger(
    ...
    require_equal(
        ledger.get("phase"),
        "latency_feasible_matched_latency",
        "ledger phase",
    )
    require_equal(
    ...
    require_selection_isolation(ledger, "ledger")
    require_equal(
    ...
    for key, expected_value in (
        ...
        (
            "contract_id",
            "dino_latency_feasible_matched_launch_20260728_v1",
        ),
...
def aggregate_bundles(
    ...
    for job in jobs:
        ...
        require_selection_isolation(
            result,
            f"{allocation_id} result",
        )
...
def build_invalidation_evidence(
    *,
    ...
    if not isinstance(source, dict):
        raise ContractError(
            "manifest lacks pinned latency-feasible aggregator provenance"
        )
    payload = {
        "schema_version": 1,
        "evidence_id": (
            "dino_latency_feasible_complete_invalid_allocation_v1"
        ),
        "status": status,
        "manifest_id": manifest["manifest_id"],
        "manifest_sha256": manifest_file_sha256,
...
def endpoint_pairwise_classification(
    directional_evidence: dict[str, Any],
    bootstrap_interval_position: str,
) -> dict[str, Any]:
    """Keep effective direction and descriptive equivalence distinct."""

    directional_claim = directional_evidence["directional_claim"]
    descriptively_equivalent = (
        bootstrap_interval_position
        == "entirely_within_practical_tolerance"
    )
    if directional_claim != "no_stable_directional_claim":
        interpretation = directional_claim
    elif descriptively_equivalent:
        interpretation = (
            "descriptively_practically_equivalent_not_confirmatory"
        )
    else:
        interpretation = "unresolved"
    return {
        "effective_directional_classification": directional_claim,
        "descriptive_practical_equivalence": descriptively_equivalent,
...
def feasible_cohort_validation_evidence(
    manifest: dict[str, Any],
    ...
    """Classify validation-only evidence for the frozen latency winner."""

    candidate_ids = sorted(
        manifest.get("candidate_derivation", {}).get("candidate_ids", [])
    )
    if len(candidate_ids) != EXPECTED_CANDIDATES:
        raise ContractError(
            "latency-feasible cohort must contain exactly four candidates"
        )
    if set(candidate_ids) != set(matched_aggregate_by_candidate):
        raise ContractError(
            "matched aggregate candidate set differs from feasible cohort"
        )
    if len(pairs) != math.comb(EXPECTED_CANDIDATES, 2):
        raise ContractError(
            "feasible-cohort analysis must contain all six unordered pairs"
        )
    ...
    require_equal(
        ...
        "latency accuracy retention",
    )
    require_equal(
        settings.get("multi_objective_min_accuracy"),
        None,
        "independent multi-objective accuracy floor",
    )
    require_equal(
        settings.get("accuracy_metric"),
        "mAP50",
    ...
    accuracy_selection = _required_mapping(
        selections.get("accuracy"),
        "frozen accuracy selection",
    )
    latency_selection = _required_mapping(
        selections.get("latency"),
        "frozen latency selection",
    )
    require_equal(
        accuracy_selection.get("winner_id"),
        FROZEN_LATENCY_WINNER_ID,
        "frozen accuracy winner",
    )
    require_equal(
        latency_selection.get("winner_id"),
        FROZEN_LATENCY_WINNER_ID,
        "frozen latency winner",
    )
    tied_ids = latency_selection.get("latency_tied_candidate_ids")
    if not isinstance(tied_ids, list) or set(tied_ids) != set(candidate_ids):
        raise ContractError(
            "frozen latency tied cohort differs from measured feasible cohort"
        )

    candidate_records = manifest.get("candidates")
    if not isinstance(candidate_records, list):
        raise ContractError("feasible-cohort candidate records are missing")
    candidates_by_id = {
    ...
    require_equal(
        ...
        set(candidate_ids),
        "feasible-cohort candidate records",
    )
    selection_time_accuracy = {}
    ...
    for candidate_id in candidate_ids:
        objectives = _required_mapping(
            ...
            f"{candidate_id} selection-time objectives",
        )
        selection_time_accuracy[candidate_id] = _finite_objective(
            objectives.get("mAP50"),
            f"{candidate_id} mAP50",
        )
        selection_time_latency[candidate_id] = _finite_objective(
            objectives.get("latency_ms"),
            f"{candidate_id} latency",
        )
    accuracy_winner_value = selection_time_accuracy[
        FROZEN_LATENCY_WINNER_ID
    ]
    retention_threshold = 0.98 * accuracy_winner_value
    feasibility = {
        candidate_id: (
            selection_time_accuracy[candidate_id]
            >= retention_threshold - accuracy_tolerance
        )
        for candidate_id in candidate_ids
    }
    if not all(feasibility.values()):
        raise ContractError(
            "manifest cohort contains a candidate below the frozen 98% floor"
        )
    highest_accuracy = max(selection_time_accuracy.values())
    if (
        selection_time_accuracy[FROZEN_LATENCY_WINNER_ID]
        < highest_accuracy - accuracy_tolerance
    ):
        raise ContractError(
            "frozen latency winner is not the highest-accuracy tied candidate"
        )

    winner_faster_than = sorted(
        edge["slower_candidate_id"]
        for edge in effective_median_edges
        if edge.get("faster_candidate_id") == FROZEN_LATENCY_WINNER_ID
    )
    expected_other_ids = sorted(
        set(candidate_ids) - {FROZEN_LATENCY_WINNER_ID}
    )
    winner_stably_fastest = winner_faster_than == expected_other_ids
    stably_faster_than_winner = sorted(
        edge["faster_candidate_id"]
        for edge in effective_median_edges
        if edge.get("slower_candidate_id") == FROZEN_LATENCY_WINNER_ID
    )
    all_median_descriptively_equivalent = all(
        item["median_effective_classification"][
            "descriptive_practical_equivalence"
        ]
        for item in pairs
    )
    all_p95_descriptively_equivalent = all(
        item["p95_effective_classification"][
            "descriptive_practical_equivalence"
        ]
        for item in pairs
    )
    all_endpoints_descriptively_equivalent = (
        all_median_descriptively_equivalent
        and all_p95_descriptively_equivalent
    )

    if winner_stably_fastest:
        outcome = "A_rec18_stably_fastest"
        interpretation = (
            "Latency mode selected the measurably fastest candidate "
            "satisfying the 98% retained-accuracy constraint."
        )
        higher_accuracy_tie_break_needed_for_practical_result = False
    elif all_endpoints_descriptively_equivalent:
        outcome = "B_all_feasible_candidates_practically_equivalent"
        interpretation = (
            "Latency mode selected the highest-accuracy member of the "
            "statistically and practically equivalent fastest feasible "
            "cohort."
        )
        higher_accuracy_tie_break_needed_for_practical_result = True
    elif stably_faster_than_winner:
        outcome = "C_another_feasible_candidate_stably_faster"
        interpretation = (
            "Validation-only remeasurement found another feasible candidate "
            "stably faster; the frozen historical selection is preserved and "
            "the selection-time/validation discrepancy requires diagnosis."
        )
        higher_accuracy_tie_break_needed_for_practical_result = None
    else:
        outcome = "D_mixed_or_unresolved"
        interpretation = (
            "The feasible candidates could not be reliably ordered under "
            "the current latency protocol."
        )
        higher_accuracy_tie_break_needed_for_practical_result = None

    return {
        "purpose": "latency_mode_validation_only",
        "outcome": outcome,
        "interpretation": interpretation,
        "frozen_latency_winner_id": FROZEN_LATENCY_WINNER_ID,
        "frozen_winner_preserved": True,
        "accuracy_metric": "mAP50",
        "latency_metric": "latency_ms",
        "latency_accuracy_retention": copy.deepcopy(
            settings["latency_accuracy_retention"]
        ),
        "accuracy_winner_value": accuracy_winner_value,
        "accuracy_retention_threshold": retention_threshold,
        "accuracy_tolerance": accuracy_tolerance,
        "practical_latency_tolerance_ms": latency_tolerance,
        "candidate_ids": candidate_ids,
        "selection_time_accuracy_by_candidate": selection_time_accuracy,
        "selection_time_latency_by_candidate_ms": selection_time_latency,
        "accuracy_feasible_by_candidate": feasibility,
        "frozen_tied_candidate_ids": sorted(tied_ids),
        "winner_stably_faster_than_candidate_ids": winner_faster_than,
        "candidate_ids_stably_faster_than_winner": stably_faster_than_winner,
        "winner_stably_fastest_by_effective_median_rule": (
            winner_stably_fastest
        ),
        "all_pairwise_median_intervals_within_tolerance": (
            all_median_descriptively_equivalent
        ),
        "all_pairwise_p95_intervals_within_tolerance": (
            all_p95_descriptively_equivalent
        ),
        "all_endpoints_descriptively_practically_equivalent": (
            all_endpoints_descriptively_equivalent
        ),
        "higher_accuracy_tie_break_needed_for_practical_result": (
            higher_accuracy_tie_break_needed_for_practical_result
        ),
        "matched_aggregate_latency_by_candidate": copy.deepcopy(
            matched_aggregate_by_candidate
        ),
        "effective_median_edges": copy.deepcopy(effective_median_edges),
        "effective_p95_edges": copy.deepcopy(effective_p95_edges),
        "classification_endpoint": "median_ms",
        "p95_role": "separately_reported_supporting_endpoint",
        "simultaneous_total_order_inference_permitted": False,
        "selector_invoked_on_matched_measurements": False,
        "selection_time_objectives_replaced": False,
        "measurements_feed_selection": False,
        "measurements_feed_reselection": False,
        "algorithm_selected_candidate_overridden": False,
        "algorithm_change_required": False,
        "algorithm_change_rule": (
            "Validation-only latency differences do not authorize reselection "
            "or an algorithm change; a reproducible correctness defect must "
...
def comparative_analysis(
    ...
    if (
        ...
        or len(raw_candidate_ids) != EXPECTED_CANDIDATES
        or len(raw_candidate_ids) != len(set(raw_candidate_ids))
    ):
        raise ContractError(
            "matched candidate IDs must be a unique four-item list"
        )
        ...
        for second in candidate_ids[first_index + 1 :]:
            ...
            pair = {
                ...
                "p95_effective_classification": (
                    endpoint_pairwise_classification(
                        p95_directional_evidence,
                        p95_classification[
                            "descriptive_bootstrap_interval_classification"
                        ],
    ...
    require_equal(
        len(pairs),
        math.comb(EXPECTED_CANDIDATES, 2),
    ...
    cohort_validation = feasible_cohort_validation_evidence(
        manifest,
        pairs,
        stable_edges,
        stable_p95_edges,
    ...
    result = {
        ...
        "latency_feasible_cohort_validation": cohort_validation,
...
def build_final_report(
    ...
    report = {
        ...
        "selection_isolation": {
            ...
            "matched_measurements_loaded_after_selector_replay": (
                selector_replay_validated is not None
            ),
            "selector_invoked_on_matched_measurements": False,
            "measurements_feed_selection": False,
            ...
            "algorithm_selected_candidate_overridden": False,
            "allowed_use": (
                "latency-mode feasible-cohort validation only"
            ),
        },
    }
...
def main() -> int:
    ...
    require_equal(
        args.output.resolve(),
        runtime_dir / "latency_feasible_matched_analysis.json",
