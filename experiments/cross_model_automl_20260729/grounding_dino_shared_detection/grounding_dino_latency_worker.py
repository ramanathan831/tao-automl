...
"""Eight-replica stabilized Grounding DINO selection-time latency worker."""
...
def _json_value(value: Any) -> Any:
    ...
    raise TypeError(
        "Grounding DINO latency metadata is not JSON serializable: "
...
def _materialize_experiment_config(
    ...
    *,
    ...
    if not isinstance(specification, dict):
        raise TypeError(
            "Grounding DINO latency specification must be a mapping"
        )
...
def main() -> int:
    ...
    from nvidia_tao_pytorch.cv.grounding_dino.dataloader.pl_odvg_data_module import (
        ODVGDataModule,
    )
    from nvidia_tao_pytorch.cv.grounding_dino.model.pl_gdino_model import (
        GDINOPlModel,
    )
    ...
    if world_size != 8 or torch.cuda.device_count() != 8:
        raise RuntimeError(
            "Grounding DINO latency requires exactly eight visible replicas"
        )
    if (
        ...
        raise RuntimeError(
            "Grounding DINO latency requires frozen offline text assets"
        )
    ...
    if (
        ...
        raise ValueError("Grounding DINO latency contract changed")
    ...
    if (
        ...
        or descriptor.get("caption")
        != "cone . forklift . cart . fire_extinguisher ."
        or not isinstance(images, list)
        or len(images) != 16
    ):
        raise ValueError(
            "unsupported Grounding DINO latency input descriptor"
        )
    if not args.checkpoint.is_file():
        raise FileNotFoundError(
            f"checkpoint is unavailable: {args.checkpoint}"
        )
    ...
    if observed_hardware != descriptor["required_hardware"]:
        raise RuntimeError(
            "Grounding DINO latency hardware changed: "
            f"{observed_hardware}"
        )
    ...
    if categories != prompts:
        raise ValueError("Grounding DINO category prompt contract changed")
    ...
    lightning_model = GDINOPlModel.load_from_checkpoint(
        str(args.checkpoint),
        map_location="cpu",
    ...
    for batch_index, batch in enumerate(data_module.test_dataloader()):
        ...
        if int(data.shape[0]) != 1 or int(data.shape[1]) != 4:
            raise RuntimeError(
                "preprocessed Grounding DINO input must be NCHW with "
    ...
    if len(preloaded) != descriptor["preloaded_batches"]:
        raise RuntimeError(
            "could not preload the frozen 16 validation batches"
        )
    token_evidence = {
        name: {
    ...
    if set(gathered_hashes) != {input_evidence["sha256"]}:
        raise RuntimeError(
            "latency replicas did not preload identical image/text inputs"
        )
    ...
    if not tao_job_id:
        raise RuntimeError(
            "TAO_JOB_ID is required for job-isolated evidence"
        )
    ...
    dist.barrier()
    if rank == 0:
        print("TAO_AUTOML_GROUNDING_DINO_LATENCY_COMPLETE", flush=True)
