...
"""Eight-replica stabilized Mask2Former selection-time latency worker.
...
def _materialize_config(specification: dict[str, Any], omega_conf: Any) -> Any:
    ...
    from nvidia_tao_pytorch.config.mask2former.default_config import (
        ExperimentConfig,
    )
...
def main() -> int:
    ...
    from nvidia_tao_pytorch.cv.mask2former.model.pl_model import (
        Mask2formerPlModule,
    )
    ...
    if world_size != 8 or torch.cuda.device_count() != 8:
        raise RuntimeError(
            "Mask2Former campaign latency requires exactly eight GPU replicas"
        )
    ...
    if (
        ...
        or contract.timed_scope != "mask2former_model_forward"
        or contract.synchronization
        != "accelerator_sync_before_and_after_each_sample"
    ):
        raise ValueError("Mask2Former campaign latency contract changed")
    ...
    if (
        ...
        raise ValueError("unsupported Mask2Former latency input descriptor")
    ...
    if hardware != descriptor["required_hardware"]:
        raise RuntimeError(f"Mask2Former latency hardware changed: {hardware}")
    ...
    config.dataset.val.batch_size = 1
    ...
    validation_dir = Path(str(config.dataset.val.img_dir))
    for item in descriptor["validation_files"]:
        if (
            isinstance(item.get("image_id"), bool)
    ...
    lightning_model = Mask2formerPlModule.load_from_checkpoint(
        str(args.checkpoint),
        map_location="cpu",
        cfg=config,
    ...
    dataset = COCODataset(
        str(config.dataset.val.instance_json),
        str(config.dataset.val.img_dir),
        cfg=config.dataset,
        is_training=False,
    )
    index_by_image_id = {
    ...
    preloaded = []
    input_batches = []
    for batch_index, source in enumerate(descriptor["validation_files"]):
        image_id = int(source["image_id"])
        if image_id not in index_by_image_id:
            raise ValueError(
                f"validation image id changed: {image_id}"
            )
        batch = dataset.collate_fn(
            [dataset[index_by_image_id[image_id]]]
        )
        ...
        if (
            ...
            raise RuntimeError(
                "preprocessed Mask2Former input shape changed: "
    ...
    def step(round_index: int, iteration: int) -> None:
        ...
        last_output = model(preloaded[linear_index % len(preloaded)])
    ...
    if last_output is None:
        raise RuntimeError("Mask2Former latency benchmark did not execute")
    ...
    dist.barrier()
    if rank == 0:
        print("TAO_AUTOML_MASK2FORMER_LATENCY_COMPLETE", flush=True)
