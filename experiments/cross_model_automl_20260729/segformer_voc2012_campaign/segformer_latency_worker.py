...
"""Eight-replica stabilized SegFormer selection-time latency worker.
...
def main() -> int:
    ...
    from nvidia_tao_pytorch.cv.segformer.dataloader.pl_segformer_data_module import (
        SFDataModule,
    )
    from nvidia_tao_pytorch.cv.segformer.model.segformer_pl_model import (
        SegFormerPlModel,
    )
    ...
    if world_size != 8 or torch.cuda.device_count() != 8:
        raise RuntimeError(
            "SegFormer campaign latency requires exactly eight GPU replicas"
        )
    ...
    if (
        ...
        or contract.timed_scope != "segformer_model_forward"
        or contract.synchronization
        != "accelerator_sync_before_and_after_each_sample"
    ):
        raise ValueError("SegFormer campaign latency contract changed")
    ...
    if (
        ...
        or descriptor.get("image_size") != 512
        or not isinstance(descriptor.get("validation_files"), list)
        or len(descriptor["validation_files"]) != 16
    ):
        raise ValueError("unsupported SegFormer latency input descriptor")
    ...
    if hardware != descriptor["required_hardware"]:
        raise RuntimeError(f"SegFormer latency hardware changed: {hardware}")
    ...
    config.dataset.segment.batch_size = 1
    config.dataset.segment.workers = 0
    config.evaluate.batch_size = 1
    root = Path(str(config.dataset.segment.root_dir))
    validation_dir = root / "images" / str(
        config.dataset.segment.test_split
    )
    ...
    data_module = SFDataModule(config.dataset.segment)
    ...
    for batch_index, batch in enumerate(data_module.test_dataloader()):
        ...
        if list(data.shape) != [1, 3, 512, 512]:
            raise RuntimeError(
                "preprocessed SegFormer input shape changed: "
    ...
    if last_output is None:
        raise RuntimeError("SegFormer latency benchmark did not execute")
    ...
    dist.barrier()
    if rank == 0:
        print("TAO_AUTOML_SEGFORMER_LATENCY_COMPLETE", flush=True)
