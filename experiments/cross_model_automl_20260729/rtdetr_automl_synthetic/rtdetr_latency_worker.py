...
"""Eight-replica stabilized RT-DETR selection-time latency worker.

The worker benchmarks real, preprocessed validation inputs on all eight GPUs.
Checkpoint loading, dataset I/O, preprocessing, and host-to-device transfers
are outside the timed scope.  The timed callable includes RT-DETR forward and
its GPU postprocessor.
"""
...
def _json_value(value: Any) -> Any:
    ...
    raise TypeError(
        "RT-DETR latency input metadata is not JSON serializable: "
...
def main() -> int:
    ...
    from nvidia_tao_pytorch.cv.rtdetr.dataloader.pl_od_data_module import (
        ODDataModule,
    )
    from nvidia_tao_pytorch.cv.rtdetr.model.pl_rtdetr_model import (
        RTDETRPlModel,
    )
    ...
    if world_size != 8 or torch.cuda.device_count() != 8:
        raise RuntimeError(
            "RT-DETR campaign latency requires exactly eight GPU replicas"
        )
    ...
    if (
        ...
        raise ValueError("unsupported RT-DETR latency input descriptor")
    ...
    if hardware != descriptor["required_hardware"]:
        raise RuntimeError(
            f"RT-DETR latency hardware changed: {hardware}"
        )
    ...
    if (
        ...
        != descriptor["validation_annotation_sha256"]
    ):
    ...
    if image_ids != descriptor["validation_image_ids"]:
        raise ValueError("validation image order changed")

    lightning_model = RTDETRPlModel.load_from_checkpoint(
        str(args.checkpoint),
        map_location="cpu",
    ...
    for batch_index, batch in enumerate(data_module.test_dataloader()):
        ...
        expected_shape = shapes[batch_index]
        if list(data.shape) != expected_shape:
            raise RuntimeError(
                "preprocessed RT-DETR input shape changed: "
    ...
    input_evidence = {
        ...
        "input_contract_sha256": descriptor["input_contract_sha256"],
    ...
    if last_output is None:
        raise RuntimeError("RT-DETR latency benchmark did not execute")
    ...
    dist.barrier()
    if rank == 0:
        print("TAO_AUTOML_RTDETR_LATENCY_COMPLETE", flush=True)
